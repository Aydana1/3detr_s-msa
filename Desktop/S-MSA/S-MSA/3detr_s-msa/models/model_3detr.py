...
from models.s_msa import weighted_inverse_distance
...
class Model3DETR(nn.Module):
    ...
    def __init__(
        ...
        self.pre_encoder2 = pre_encoder2
        ...
        self.mlp_projection = GenericMLP(input_dim=3, hidden_dims=[128], output_dim=256)
    ...
    def run_encoder(self, point_clouds):
        ...
        #print(pre_enc_features.shape) #torch.Size([2, 256, 2048])
    ...
    def forward(self, inputs, encoder_only=False):
        ...
        point_features = enc_xyz # [2, 1024, 3] #enc_features.permute(1, 0, 2)
    
        xyz, features = self._break_up_pc(point_clouds)
        pre_enc_xyz2, pre_enc_features2, pre_enc_inds2 = self.pre_encoder2(xyz, features)
        sampled_points = pre_enc_xyz2 #.permute(0, 2, 1)  #[2, 2048, 3]
        upsampled_feats = torch.empty_like(sampled_points)

        #for i in range(point_features.shape[0]):
            
            # Instantiate NearestNeighbors class with n_neighbors=3
            
        for k in range(sampled_points.shape[0]):  #iterate over the batch size

            data = point_features[k].cpu().detach().numpy()
            nbrs = NearestNeighbors(n_neighbors=3, algorithm='ball_tree').fit(data)
            
            query_points = sampled_points[k].cpu().detach().numpy()
            d, indices = nbrs.kneighbors(query_points, n_neighbors=3) #return_distance=False
            neighbors = data[indices]
            nearest_points = torch.from_numpy(neighbors)
            nearest_points = nearest_points.permute(1, 0, 2) #(3, 2048, 3)   3 nearest points and (2048, 3)
            nearest_points = nearest_points.cpu().detach().numpy()

            wid_val = weighted_inverse_distance(nearest_points, query_points, power=2)
            upsampled_feats[k] = torch.from_numpy(wid_val)
                
        #print("umsp features ", upsampled_feats.shape) torch.Size([8, 2048, 3])
        # Project 3 to 256 using MLP
        upsampled_feats2 = self.mlp_projection(upsampled_feats)

        # Apply cross-attention between (upsampled_feats2 and pre_enc_feats/enc_feats...?) and the queries (cluster).
        # in the 1st layer of decoder
        # pass concat of encoder and upsampled features
        upsampled_feats2 = upsampled_feats2.permute(1, 0, 2)  # interpolated features with 2x points
        merged_feats = torch.cat((enc_features, upsampled_feats2), 0) # pass this to decoder instead of encoder features
        ...
        #print("enc pos: ", enc_pos.shape)

        ## encoder xyz: npoints x batch x 3
        sampled_pos = self.pos_embedding(sampled_points, input_range=point_cloud_dims)
        #print("sample pos: ", sampled_pos.shape)
        merged_pos = torch.cat((enc_pos, sampled_pos), 2)
        merged_pos = merged_pos.permute(2, 0, 1)
        #print("mrg pos: ", merged_pos.shape)
        ...
        box_features = self.decoder(
            tgt, merged_feats, query_pos=query_embed, pos=merged_pos #enc_pos
        )[0]  #enc_features
...
def build_preencoder2(args):
    mlp_dims = [3 * int(args.use_color), 64, 128, args.enc_dim]
    preencoder = PointnetSAModuleVotes(
        radius=0.2,
        nsample=64,
        npoint=args.preenc_npoints*2,  #2N points
        mlp=mlp_dims,
        normalize_xyz=True,
    )
    return preencoder
...
def build_3detr(args, dataset_config):
    ...
    pre_encoder2 = build_preencoder2(args)
