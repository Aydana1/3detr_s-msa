    ...
    def forward(self, tgt, memory,
        ...
        for layer in self.layers:
            ...
            output, attn = layer(output, memory, tgt_mask=tgt_mask,
                           ...
                           return_attn_weights=return_attn_weights, layer_id = layers_count)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=128,
        ...
        self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout_attn) #nn.MultiheadAttention(d_model, nhead, dropout=dropout_attn)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=256,
        ...
        if dropout_attn is None:
            dropout_attn = dropout
        self.self_attn = MultiheadAttention(d_model, nhead, dropout=dropout) #nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn = MultiheadAttention(d_model, nhead, dropout=dropout) #nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn2 = MultiheadAttention(d_model, nhead, dropout=dropout) #nhead // 2
        ...
        ##
        self.linear3 = nn.Linear(512, dim_feedforward)
        self.linear4 = nn.Linear(dim_feedforward, 256)
    ...
    def forward_pre(self, tgt, memory,
        ...
        #merged feats
        s_msa = True
        #print("pos: ", pos.shape) # torch.Size([3072, 2, 256])
        if layer_id != 1:  #in layer 1, memory consists of merged features
            s_msa = False
            memory = torch.split(memory, [2048, 4096])  #  2048, 4096
            memory = memory[0]  #([1024, 2, 256]
            memory2 = memory[1] #[2048, 2, 256]
            pos = torch.split(pos, [2048, 4096])   # 2048, 4096
            pos = pos[0]
            pos2 = pos[1]
        ####
        #print("memory: ", memory.shape) # torch.Size([3072, 2, 256])
       # tgt4 = tgt
        #print("tgt: ", tgt.shape)
        ...
        tgt2, attn = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask, s_msa=s_msa)
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
    ...
    def forward(self, tgt, memory,
        ...
        if self.normalize_before:
            return self.forward_pre(tgt, memory, tgt_mask, memory_mask,
                                    tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos, return_attn_weights, layer_id)
